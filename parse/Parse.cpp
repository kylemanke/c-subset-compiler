...
, mNeedPrintf(false)
, mCheckSemant(true) // enabled semantic checking
...
Identifier* Parser::getVariable(const char* name) noexcept
{
	...
	if(!ident) {
		// get dummy and report error
		ident = mSymbols.getIdentifier("@@variable");
		std::string msg = "Use of undeclared identifier '" + std::string(name) + "'";
...
std::shared_ptr<ASTExpr> Parser::charToInt(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	if(expr) {
		// If not int must do some work
		if(expr->getType() != Type::Int && expr->getType() == Type::Char) {
			// Check if constant expression
			if(dynamic_cast<ASTConstantExpr*>(expr.get())) {
				dynamic_cast<ASTConstantExpr*>(expr.get())->changeToInt();
			} // Check if ASTToCharExpr
			else if(dynamic_cast<ASTToCharExpr*>(expr.get())) {
				retVal = dynamic_cast<ASTToCharExpr*>(expr.get())->getChild();
			}
			else {
				std::shared_ptr<ASTToIntExpr> parent = make_shared<ASTToIntExpr>(expr);
				retVal = parent;
			}
		}
	}
...
std::shared_ptr<ASTExpr> Parser::intToChar(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	if(expr) {
		// Check if already char
		if(expr->getType() != Type::Char && expr->getType() == Type::Int) {
			// Check if constant expression
			if(dynamic_cast<ASTConstantExpr*>(expr.get())) {
				dynamic_cast<ASTConstantExpr*>(expr.get())->changeToChar();
			} // Check if ASTToIntExpr
			else if (dynamic_cast<ASTToIntExpr*>(expr.get())) {
				retVal = dynamic_cast<ASTToIntExpr*>(expr.get())->getChild();
			}
			else {
				std::shared_ptr<ASTToCharExpr> parent = make_shared<ASTToCharExpr>(expr);
				retVal = parent;
			}
		}
	}
